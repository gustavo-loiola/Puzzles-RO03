# =============================================================================
#  Mosaic Puzzle — Full Runner Script
#  Run from the mosaic/ directory:  julia test_mosaic.jl
# =============================================================================

# --- Setup ---
# include() resolves relative to the file doing the include (mosaic/)
# cd() affects runtime I/O functions (readdir, open, etc.)
include(joinpath(@__DIR__, "src", "resolution.jl"))
cd(joinpath(@__DIR__, "src"))

# ═══════════════════════════════════════════════════════════════════════════════
#  PART 1 — Solve the hand-crafted instances with CPLEX
# ═══════════════════════════════════════════════════════════════════════════════
println("=" ^ 70)
    ...
    if isOpt
        ...
        if all(expected .!= -1)
            if sol == expected
                ...
                println("  X MISMATCH with expected answer!")
            end
        end
    end
end
...
for file in sort(filter(x -> occursin(".txt", x), readdir("../data/")))
    ...
    # Try the heuristic repeatedly (up to 10s)
...
# Performance diagram (requires at least one solved instance in res/)
try
